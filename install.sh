...
if [ -d "$DESKTOP_DIR" ]; then
    ...
    echo "🖥️ Desktop shortcut created at $DESKTOP_FILE"
else
    echo "⚠️ Desktop directory not found."
fi

# Register as system application
mkdir -p "$APPS_DIR"
APP_MENU_FILE="$APPS_DIR/readaloud.desktop"
echo "$DESKTOP_ENTRY" > "$APP_MENU_FILE"
chmod +x "$APP_MENU_FILE"
if command -v update-desktop-database &> /dev/null; then
    update-desktop-database "$APPS_DIR" 2>/dev/null || true
fi
echo "icon Registered ReadAloud in the system applications menu ($APP_MENU_FILE)"
